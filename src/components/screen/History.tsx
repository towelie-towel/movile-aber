...
import { useColorScheme, View, useWindowDimensions, Text, ScrollView, Platform, Pressable } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { MaterialCommunityIcons, FontAwesome6 } from '@expo/vector-icons';
import { Path, Svg } from 'react-native-svg';
import { BlurView } from 'expo-blur';

import { AbsoluteDropdown, ScaleBtn } from '~/components/common';
...
const HistoryScreen = () => {
    ...
    return (
        <Pressable onPress={() => setIsDropDownOpen(false)} className="flex-1" style={{ backgroundColor: Colors[colorScheme ?? 'light'].background_light }}>
            {Platform.OS === 'ios' && <BlurView style={{ position: 'absolute', zIndex: 1000, height: insets.top, width, top: 0 }} tint="light" intensity={20} />}
            <View className={"absolute top-0 pt-20 justify-end z-0"} style={{ backgroundColor: Colors[colorScheme ?? 'light'].primary, }}>
            ...
            <View className='flex-1 relative z-10 bg-transparent'>
                <View className='flex-row justify-between items-end bg-transparent h-40 w-full px-[5%] pb-4 relative z-40'>
                    <View className=''>
                        <FontAwesome6 name="chevron-left" size={28} color={Colors[colorScheme ?? "light"].border_light_i} />
                        <Text style={{ color: Colors[colorScheme ?? "light"].border_light_i }} className='font-bold text-[2.5rem]'>History</Text>
                    </View>
                    <View className=''>
                        {/* Here should go the date picker */}
                    </View>
                </View>
                <ScrollView showsVerticalScrollIndicator={false} className="px-[5%] overflow-visible">
                    {
                        TestRidesData.map((item) => {
                            return (
                                <View key={item.id} className='mb-6 rounded-lg shadow-sm' style={{ backgroundColor: Colors[colorScheme ?? 'light'].background_light }} >
                                    <View className='px-2'>
