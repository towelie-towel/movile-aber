...
export const isValidPhone = (phoneNumber: string, errors = {
    ...
    if (!phoneNumber.startsWith(errors.starts_with.value) && errors.starts_with.active) {
        return [false, errors.starts_with.message.replace("<value>", errors.starts_with.value.toString())];
    }
...
export const isValidPassword = (password: string, errors = {
    ...
    if ((password.length > errors.max_length.value) && errors.max_length.active) {
        return [false, errors.max_length.message.replace("<value>", errors.max_length.value.toString())];
    }

    if ((password.length < errors.min_length.value) && errors.min_length.active) {
        return [false, errors.min_length.message.replace("<value>", errors.min_length.value.toString())];
    }
