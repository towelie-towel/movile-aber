...
import Animated, { scrollTo, useWorkletCallback } from 'react-native-reanimated';
...
export const useCustomScrollEventsHandlers: ScrollEventsHandlersHookType = (
  ...
  const handleOnScroll: ScrollEventHandlerCallbackType<ScrollEventContextType> = useWorkletCallback(
    (_, context) => {
      const { didDragBelowSecondSnapPoint, isDraggingDownFromMiddle, isDraggingDownFromTop } =
        getScrollMetadata({
          context,
          animatedSnapPoints,
          gestureTranslationY,
  ...
  const handleOnEndDrag: ScrollEventHandlerCallbackType<ScrollEventContextType> =
    useWorkletCallback(
      ({ contentOffset: { y } }, context) => {
        const { didStartAtMiddle, didDragBelowSecondSnapPoint, isDraggingDownFromTop } =
          getScrollMetadata({
            context,
            animatedSnapPoints,
            gestureTranslationY,
