...
// Placeholder clamp function
function clamp(value, lowerBound, upperBound) {
  return Math.min(Math.max(value, lowerBound), upperBound);
}

// Placeholder snapPoint function
function snapPoint(value, velocity, snapPoints) {
  const minDistance = snapPoints.reduce((minDist, point) => {
    const distance = Math.abs(point - value);
    return distance < minDist ? distance : minDist;
  }, Number.MAX_VALUE);

  const nearestPoint = snapPoints.find((point) => Math.abs(point - value) === minDistance);

  // Apply some logic to determine the new value based on velocity if needed
...
export const useCustomGestureEventsHandlers = () => {
  ...
  const handleOnActive: GestureEventHandlerCallbackType = useWorkletCallback(
    function handleOnActive(source, { translationY }, context) {
      ...
      let highestSnapPoint = animatedSnapPoints.value[animatedSnapPoints.value.length - 1];
      /**
       ...
       */
      const secondHighestSnapPoint = animatedSnapPoints.value[animatedSnapPoints.value.length - 2];
      const isDraggingFromBottom = context.initialPosition > secondHighestSnapPoint;
      ...
      const clampedPosition = (() => {
        if (source === GESTURE_SOURCE.SCROLLABLE) {
          ...
          return clamp(accumulatedDraggedPosition, highestSnapPoint, lowestSnapPoint);
        }
      })();
      ...
      if (enableOverDrag) {
        ...
        if (source === GESTURE_SOURCE.HANDLE && draggedPosition > lowestSnapPoint) {
          const resistedPosition =
            lowestSnapPoint +
            Math.sqrt(1 + (draggedPosition - lowestSnapPoint)) * overDragResistanceFactor;
          animatedPosition.value = resistedPosition;
          return;
        }
      }
